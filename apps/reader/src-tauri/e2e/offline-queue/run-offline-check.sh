#!/usr/bin/env bash
# Deterministic offline write-queue check (N2.3). No Tauri, no engine, no network.
# esbuild-bundles the REAL @komika/api and runs offline-queue.check.ts under node,
# printing PASS/FAIL per assertion and exiting non-zero on any failure.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"        # .../e2e/offline-queue
REPO="$(cd "$SCRIPT_DIR/../../../../.." && pwd)"                   # repo root
OUT="$SCRIPT_DIR/build"                                            # generated bundle (gitignored)
mkdir -p "$OUT"

# esbuild resolver: mirror native-read/run.sh — probe the hoisted .bin, else npx.
...
BUNDLE="$OUT/offline-queue.check.mjs"
...
"${ESBUILD[@]}" "$SCRIPT_DIR/offline-queue.check.ts" \
