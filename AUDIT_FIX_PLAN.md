...
- [x] 5.1 🟡 SC1 — `poll_every_minutes` is now live. Added it to `ScanAdmin`'s select and a clamped `resolve_poll_minutes()` (floor `MIN_POLL_MINUTES = 15`; capped at the steady interval so a poll can't be slower than steady). "Awaiting" = a non-first scan that came due and found no new chapter → `next_scan_at = now + poll` instead of `+ steady`; a landed chapter (or first observation) reverts to steady. **Gating switched from the live `is_overdue`/effective-interval recompute to the persisted `next_scan_at`** (`is_due(next_scan_at, now)`), which is what carries the accelerated poll window into the tick — and this also closes **SC7** (gate and the admin console's "next due" now read the same stored value). This supersedes the tick-side clamp from 5.4; the SC5 clamp that matters now lives in the single `resolve_interval` that computes the persisted `next_scan_at`. Removed the now-dead `is_overdue` + its 3 tests; added `is_due` (3), `resolve_poll_minutes` (1), and a DB test asserting the awaiting→30m→revert→168h cadence. **Caveat:** end-to-end freshness still depends on Suwayomi's `fetchChapters:true` actually forcing a source refresh (an upstream black box, out of scope) — this fixes the scheduling/bookkeeping, not upstream staleness. **Post-review hardening (migration `0013`, `awaiting_since`):** the accelerated poll is now (a) **bounded** — it runs only within `min(steady_interval, AWAITING_MAX_HOURS = 48h)` past the due time, then falls back to steady, so a stalled-but-ONGOING or interval-underestimated series can't poll every 30m forever; and (b) **entered only when genuinely due** (`is_due(prior.next_scan_at)`), so an admin `triggerScan` that force-scans *before* the cadence no longer flips the series into poll mode. `awaiting_since` is stamped on entry, preserved across polls, cleared when a chapter lands. +2 DB tests (early-manual-scan stays steady; awaiting backs off after the window). **Live-verified** against a real Suwayomi v2.3.2232 container (MangaDex source, Oshi no Ko): `library()`/`series()`/`chapters()` deserialize against the real schema; the real millisecond `uploadDate` strings feed `avg_interval_hours` (≈416h); and `record_scan` over the real 102-chapter/max-#166.5 list confirms baseline (SC3), the count-vs-max-number divergence (SC4), idempotency (SC6), and number-advance detection — see the `#[ignore]`d `live_suwayomi_end_to_end` test (run with `KOMIKA_LIVE_SUWAYOMI=...`). The `fetchChapters:true` source-refresh semantics remain an upstream detail, but the bookkeeping is now confirmed against real data, not just synthetic structs.
...
- [x] 5.4 🟡 SC5 — added `MIN_INTERVAL_HOURS = 6.0` and a shared `resolve_interval(override, inferred)` used in **both** the tick gate and the `record_scan` next-interval recompute. MIN floor applies only to the *inferred* avg (floors sub-hour burst avgs so they aren't refetched every 300s tick); a deliberate admin override may go below MIN but is still clamped to a `HARD_MIN_INTERVAL_HOURS = 1.0` hard floor to protect upstreams. Upper `MAX` clamp preserved. 2 pure tests.
- [x] 5.5 ⚪ SC6 / SC7 — **SC6:** `record_scan` now opens a `pool.begin()` transaction, reads `prior` and writes the upsert inside it, and commits — the slow chapter fetch already happened in `scan_series`, so the tx spans only the two DB ops. An admin `triggerScan` overlapping a tick can no longer interleave read-modify-write; under WAL a losing concurrent writer gets `SQLITE_BUSY_SNAPSHOT` and errors out (logged + skipped, retried next interval) instead of clobbering. Idempotency test added (repeated identical scan doesn't double-count or re-flag). **SC7:** already closed by 5.1 — the tick gate and the console's "next due" both read the persisted `next_scan_at`, so they can't disagree (confirmed, no extra change). **Phase 5 fully closed.**
...
- [x] 6.3 ⚪ CR4 — number-less chapter ordering (server/reader agreement) — **option (a)** (reader, canonical-only): stop re-sorting by number on the canonical branch and consume the server's ascending/number-less-last order. `getReaderChapter` uses `asc = canonical ? chs : sort(...)`; `mapSeriesView` gains a `preserveOrder` flag passed true from the canonical `getSeries` branch. Suwayomi paths and `getLibrary:277` untouched; svelte-check 0/0. **Follow-up:** the exact ch-0-vs-oneshot *collision* (both arrive as wire `0`) is NOT resolved by (a) — only option (b) (nullable `Chapter.number` end-to-end) fixes it, and it needs the shared contract change; left as a noted follow-up per the finding's cosmetic rating.
...
- **6.4 CR6 → dedicated prompt: [`docs/audit-fixes/PROMPT-6.4-canonical-progress.md`](docs/audit-fixes/PROMPT-6.4-canonical-progress.md).**
  ...
  **DONE** (migration landed as `0014_canonical_progress.sql`, not `0010` — Phase 5 had advanced the sequence
  to `0013`). The Library-screen merge remains the noted deferred follow-up.
